...
{
    if [ "$DOMAIN" ]; then
        cp "/swag/nginx/proxy-confs/kibana.subdomain.conf.sample" "/swag/nginx/proxy-confs/$DOMAIN.subdomain.conf"
        #mv "/swag/nginx/proxy-confs/kibana.subdomain.conf.sample" "/swag/nginx/proxy-confs/$DOMAIN.subdomain.conf"
        sed -i -e "s/REPLACE_ME.*;/$DOMAIN;/" "/swag/nginx/proxy-confs/$DOMAIN.subdomain.conf"
    elif [ "$SUBFOLDER" ]; then
        ...
        echo "No DOMAIN or SUBFOLDER variable set.... skipping ...."
    fi
}
